...
class Thinker(BaseThinker):
    ...
    def __init__(
        ...
        # Whether the active model has already been updated for this batch
        ...
        self.to_audit: dict[int, Trajectory] = {}
    ...
    def _select_structures(self, structures: list[ase.Atoms], energies: np.ndarray, traj_ids: list[int]) \
        ...
        output = []
        # Need at least a few structures to pick from
    ...
    def store_simulation(self, result: Result):
        ...
        if result.success:
            ...
            # 10 eV/atom is much larger than our typical error
            self.audit_results.append(10 / traj.last_run_length)
...
if __name__ == '__main__':
    ...
    #     import my_util.multi_node_config as make_config
    #     from colmena.task_server import ParslTaskServer

    #     if args.parsl_site == "theta-venti":
    #         # Create the resource configuration
    #         # config = parsl_configs.theta_debug_and_venti(str(out_dir))
    #         config, node_resources = make_config(out_dir / "slurm_resources.ini", str(out_dir))
    ...
    #         # set all available executors here, choose appropriate executor in evo
